...
export class SpreadService {
  ...
  async parseSpread(cols, rows, table, api42s?) {
    ...
    for (const row of rows) {
      ...
      tuple['fk_user_no'] = row[1];
      //      console.log(tuple, 'tupleeee');
      //    console.log(`tablename: ${tableName}`);
      tupleLine = table['repo'].create(tuple);
      //  console.log(tupleLine, 'lidldidl');
      await table['repo'].save(tupleLine);
  ...
  async composeTableData(
    ...
    this.makeTableSet(tableSet, endOfTables, repoArrays, mapObj, tableNames);
  }
  ...
  makeTableSet(
    ...
    for (const tableName in tableNames) {
      ...
      table['repo'] = repoArrays[tableName];
